...
/* void t_coproc    // 9 */	// reserved (not generated by recent processors)
...
page_fault_handler(struct Trapframe *tf)
{
  ...
  if (curenv->env_pgfault_upcall == NULL || 
      ...
      tf->tf_esp < (UXSTACKTOP - PGSIZE + sizeof(struct UTrapframe) + 4)) {
    cprintf("[%08x] user fualt va %08x ip %08x\n",
  ...
  if (tf->tf_esp <= USTACKTOP) {
    uxstacktop = USTACKTOP - sizeof(struct UTrapframe);
  } else {
    uxstacktop = tf->tf_esp - 4 - sizeof(struct UTrapframe);
  }

  user_mem_assert(curenv, (void *)uxstacktop, 1, PTE_W | PTE_U);
  struct UTrapframe* u_tf = (struct UTrapframe*)uxstacktop;
  ...
  curenv->env_tf.tf_esp = (uintptr_t) uxstacktop;
